...
class funcs
{
    ...
    async add_category(__id: number, __name: string)
    {
        ...
        if(await Category.find({id_category: __id})) return 'ID is busy!';
    ...
    {
        let status: string = 'Ok!';
        if(await Customer.find({id_customer: __id})) return 'ID is busy!';
    ...
    {
        let status: string = 'Ok!';
        if(await Type.find({id_model: __id})) return 'ID is busy!';
    ...
    {
        let status: string = 'Ok!';
        if(await Order.find({id_order: __id})) return 'ID is busy!';
