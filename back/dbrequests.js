...
class funcs {
    ...
    async sum_for_period(__id, start, end) {
        let res = 0;
        let query = await Order.find({ type: __id, date_of_order: { "$gte": start, "$lte": end } }, function (err, orders) { });
        let price_per_one;
        await Type.findOne({ id_model: __id }, function (err, type) {
            price_per_one = type === null || type === void 0 ? void 0 : type.price;
        });
        let i;
        for (i of query) {
            res += price_per_one;
    ...
    async add_model(__id, __name, __price, __category, __storage) {
        let id_status = true;
        let cat_status = false;
        if (await Type.exists({ id_model: __id }))
            id_status = false;
        if (await Category.exists({ id_category: __category }))
            cat_status = true;
        if (id_status && cat_status)
